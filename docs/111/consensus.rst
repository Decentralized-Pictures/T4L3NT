...
header<emmyp_fitness_and_header_111>` contains, most notably, a timestamp, a
...
of baking slots and a list of ``ENDORSERS_PER_BLOCK`` endorsing slots (the value of ``ENDORSERS_PER_BLOCK`` is one of the :ref:`parameters of the consensus protocol <cs_constants_111>`). The index
...
of participants is at :ref:`random<emmyp_slot_selection_111>`, independently for
...
- A participant always adopts the :ref:`fittest<emmyp_fitness_and_header_111>`, that
...
:ref:`roll<roll_pos_111>`, and is :ref:`active<active_delegate_111>`. For simplicity we
...
:ref:`here<rights_111>`. The mechanism behind baking slots is meant to ensure that
...
- the protocol-specific header of a block has the following fields:
  ...
  - ``seed_nonce_hash``: a commitment to :ref:`a random number<random_seed_111>`, used to
    generate entropy on the chain. Present in only one out of
    ``BLOCKS_PER_COMMITMENT`` (see :ref:`Constants<ps_constants_111>`).
...
values.
Note that these parameters are part of the larger set of :ref:`protocol constants <protocol_constants_111>`.
...
blocks<ps_constants_111>`, a cycle lasts *at least* 2 days, 20 hours, and 16
...
function<delaystar_111>` says that:
